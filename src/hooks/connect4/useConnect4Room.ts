...
/**
 * Manage a Connect4 room: join a room, handle player matching, and synchronize the initial turn using Socket.IO.
 *
 * @param roomId - ID of the room to join
 * @param setFirstRole - Setter that updates the initial-turn state for the match
 * @param firstRole - Current initial-turn state (`'random'` or a `Role`)
 * @returns An object containing:
 * - `socketRef`: Ref to the Socket.IO client
 * - `members`: Current number of members in the room
 * - `playerRole`: This player's assigned role or `null` if not assigned
 * - `matchState`: Current match status (`"waiting" | "matched" | "playing"`)
 * - `setMatchState`: Setter to update `matchState`
 * - `membersRef`: Ref containing the current members count for stable access in callbacks
 * - `emitRestart`: Function that emits a restart event for the room
